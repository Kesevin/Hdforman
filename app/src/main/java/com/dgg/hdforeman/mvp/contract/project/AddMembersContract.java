...
public interface AddMembersContract {
    ...
    interface View extends ListBaseView {
        void bindDataToView(List<TeamBean> teamList);
        void removeTeamList();
    }

    //Model层定义接口,外部只需关心model返回的数据,无需关心内部细节,及是否使用缓存
    interface Model {
        List<TeamBean> getAddMembersListData(List<Team> list,List<ConstructTeamBean> mPreList);
        Observable<BaseData<List<Team>>> getAddMembersListDatas();
        Observable<BaseData> addTeamMember(String proId,String workId);
    }

    interface NetUtil{
        void addTeamMemberUtil(TeamBean teamBean);
    }
}