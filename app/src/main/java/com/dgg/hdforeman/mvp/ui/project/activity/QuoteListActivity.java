...
import static com.jess.arms.utils.CharactorHandler.emojiFilter;
...
public class QuoteListActivity extends BacksActivity<QuoteListPresenter> implements QuoteListContract.View {
    ...
    protected void initData() {
        mEtQuoteListRemark.setFilters(new InputFilter[]{emojiFilter});
...
//                            Date afterdate = new Date(System.currentTimeMillis() + 365 * 24 * 60 * 60 * 1000 * 2);
//                            if (date.after(afterdate)) {
//                                UiUtils.makeText("亲～，暂时不能预约两年后的业务哟");
//                                return;
//                            }

                            int daecompareBefore = TimeUtil.compareDate(date, calendar.getTime());
                            if (daecompareBefore == -1) {
                                UiUtils.makeText("亲～，暂时不能预约两年后的业务哟");
                                return;
                            }
