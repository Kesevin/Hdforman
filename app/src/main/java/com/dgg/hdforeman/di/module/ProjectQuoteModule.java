...
import com.dgg.hdforeman.mvp.model.project.ProjectQuoteModel;
...
@Module
public class ProjectQuoteModule {
    private ProjectQuoteContract.View view;

    /**
     * 构建ProjectQuoteModule时,将View的实现类传进来,这样就可以提供View的实现类给presenter
     * @param view
     */
    public ProjectQuoteModule(ProjectQuoteContract.View view) {
        this.view = view;
    }

    @ActivityScope
    @Provides
    ProjectQuoteContract.View provideProjectQuoteView() {
        return this.view;
    }

    @ActivityScope
    @Provides
    ProjectQuoteContract.Model provideProjectQuoteModel(ServiceManager serviceManager, CacheManager cacheManager
            , Gson gson, Application application, DaoSession daoSession) {
        return new ProjectQuoteModel(serviceManager, cacheManager, gson, application,daoSession);
    }

    @ActivityScope
    @Provides
    ProjectMessureResultContract.Model provideProjectMessureResultModel(ServiceManager serviceManager
