...
public class ProjectWaitToStartDetailModel extends BaseModel<ServiceManager, CacheManager> implements ProjectWaitToStartDetailContract.Model {
    private Gson mGson;
    private Application mApplication;
    private CommonCache mCommonCache;
    private CommonService mCommonService;

    public ProjectWaitToStartDetailModel(ServiceManager serviceManager, CacheManager cacheManager, Gson gson, Application application) {
    ...
    public Observable<BaseJson<ProjectInfoResponse>> getProjectInformation(@Nullable String proid) {
        ...
        params.setD(request);
        CommonUtil.logDebug(mGson.toJson(params));
        return mCommonService.getProjectInfo(mGson.toJson(params));
    }

    @Override
    public Observable<BaseJson<ProjectShutResponse>> StartProject(@Nullable String proid) {
        BaseParams<ProjectInfoRequest> params = new BaseParams<>();
        params.setC(Api.RequestModule.getProAction.getClassName());
        params.setM(Api.RequestModule.getProAction.getMethodName());
        ProjectInfoRequest request = new ProjectInfoRequest();
        request.setProid(proid);
        params.setD(request);
        CommonUtil.logDebug(mGson.toJson(params));
        return mCommonService.StartWork(mGson.toJson(params));
    }
}