    ...
    public void initAdapter() {
        ...
        mAdapter.setOnItemClickListener(new BaseAdapter.OnRecyclerViewItemClickListener<ProjectAcceptResponse>() {
            ...
            public void onItemClick(View view, ProjectAcceptResponse data, int position) {
                if (data.getPs_state() == 0) {
                    mRootView.showMessage("当前阶段未完工，不能操作!");
                    return;
                }

                if (data.getPs_state() == 1) {
                    mRootView.showMessage("当前阶段施工中，不能操作!");
                    return;
                }

                if (data.getPs_state() == 2) {
                    mRootView.showMessage("当前阶段已停工，不能操作!");
                    return;
                }

                if (data.getIsAccept() == 0) {
                    mRootView.showMessage("当前阶段监理未验收，不能操作!");
                    return;
                }
